...
export class MobileFastlyCachePurger extends GuStack {
	constructor(scope: App, id: string, props: GuStackProps) {
		...
		const faciaID =
			this.stage == 'CODE'
				? 'StorageConsumerRole-1JWVQ2NTELFT7'
				: 'StorageConsumerRole-1R9GQEVJIM323';
				...
				logs: new iam.PolicyDocument({
					statements: [
						...
						new iam.PolicyStatement({
							...
							resources: [
								`arn:aws:logs:eu-west-1:${this.account}:log-group:/aws/lambda/*:*`,
							],
						}),
					],
				}),
				Conf: new iam.PolicyDocument({
					statements: [
						new iam.PolicyStatement({
							...
							resources: [
								`arn:aws:ssm:${this.region}:${this.account}:parameter/cache-purger/${this.stage}`,
							],
						}),
					],
				}),
				Assume: new iam.PolicyDocument({
					statements: [
						new iam.PolicyStatement({
							...
							resources: [
								`arn:aws:iam::${GuardianAwsAccounts.CMSFronts}:role/facia-${this.stage}-${faciaID}`,
							],
						}),
					],
				}),
		...
		const imageRepositoryArn = Fn.importValue(
			'mobile-fastly-cache-purger-repository-arn',
		);
		const imageRepositoryName = Fn.importValue(
			'mobile-fastly-cache-purger-repository-name',
		);
		const handler = new GuLambdaDockerFunction(
			this,
			'mobile-fastly-cache-purger-v2',
			{
		...
		const frontsUpdateTopicName =
			this.stage == 'CODE'
				? 'FrontsUpdateSNSTopic-RepwK3g95V3w'
				: 'FrontsUpdateSNSTopic-kWN6oX2kvOmI';
		...
		const eventSource: lambdaEventSources.SqsEventSource =
